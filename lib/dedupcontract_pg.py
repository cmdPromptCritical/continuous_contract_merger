...
It sums volume per symbol_period using the one_day_candle materialized view, reports volume by source to the terminal,
...
def process_day(conn, day: date, symbol: str, delete: bool = False, dry_run: bool = False):
    ...
    with conn.cursor() as cur:
        # 1) Get volumes per symbol_period from one_day_candle materialized view
        cur.execute("""
            SELECT symbol_period, day_volume
            FROM one_day_candle
            WHERE time_bucket::date = %s AND symbol = %s
            ORDER BY symbol_period;
        """, (day_str, symbol))
        rows = cur.fetchall()
        if not rows:
            print(f"[{day_str}] No candle data found for symbol {symbol}.")
            return
        ...
        max_vol = max(vol for _, vol in rows)
        max_sources = [src for src, vol in rows if vol == max_vol]
        print(f"    Highest volume = {max_vol} from source(s): {', '.join(max_sources)}")

        # Identify sources to delete (all except max_sources)
        sources_to_delete = [src for src, _ in rows if src not in max_sources]

        # 4) Delete trades if requested
        if delete and sources_to_delete:
            placeholders = ", ".join(["%s"] * len(sources_to_delete))
            del_sql = f"""
                DELETE FROM market_data
                 WHERE trade_date = %s AND symbol = %s
                   AND symbol_period IN ({placeholders});
            """
            params = [day_str, symbol] + sources_to_delete
            print(f"    → Deleting trades for {len(sources_to_delete)} source(s)...")
            if not dry_run:
