"""
QuestDB Trades Deduplication Script

This script deduplicates trading data in QuestDB by identifying and marking the
front contract for each symbol on each trading day. For each symbol and date,
it sets the 'front_contract' flag to True for the symbol_period with the highest
total volume, and False for all others.

Usage:
  python deduplicate_questdb.py [--date YYYY-MM-DD] [--symbol SYMBOL]
...
Examples:
  python deduplicate_questdb.py
  python deduplicate_questdb.py --date 2024-01-15
  python deduplicate_questdb.py --symbol AAPL
  python deduplicate_questdb.py --date 2024-01-15 --symbol AAPL
